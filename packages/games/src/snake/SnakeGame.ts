...
export class SnakeGame extends Phaser.Scene {
  ...
  private setupInput() {
    ...
    this.input.keyboard?.on('keydown', (event: KeyboardEvent) => {
      ...
      switch (event.code) {
        case 'ArrowUp':
        case 'KeyW':
          if (this.gameRunning && this.direction.y !== 1) this.nextDirection = { x: 0, y: -1 }
          handled = true
          break
        case 'ArrowDown':
        case 'KeyS':
        ...
        case 'ArrowLeft':
        case 'KeyA':
          if (this.gameRunning && this.direction.x !== 1) this.nextDirection = { x: -1, y: 0 }
          handled = true
          break
        case 'ArrowRight':
        case 'KeyD':
  ...
  private render() {
    ...
    this.add.rectangle(
      ...
      this.config.gridSize - 2, 
      parseInt(this.config.foodColor.replace('#', ''), 16)
    )
...
export function init(container: string, config: SnakeConfig = {}, callbacks: GameCallbacks = {}) {
