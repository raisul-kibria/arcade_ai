...
export class TetrisGame extends Phaser.Scene {
    ...
    create() {
        // Initialize board
        this.board = Array(this.config.gridHeight).fill(null).map(() => Array(this.config.gridWidth).fill(0));
        this.score = 0;
        this.lines = 0;
        this.level = 1;
    ...
    movePiece(dx, dy) {
        if (!this.currentPiece)
            return false;
        const newPiece = Object.assign(Object.assign({}, this.currentPiece), { x: this.currentPiece.x + dx, y: this.currentPiece.y + dy });
    ...
    rotatePiece() {
        ...
        const newPiece = Object.assign(Object.assign({}, this.currentPiece), { shape: rotated });
        if (this.isValidPosition(newPiece)) {
            this.currentPiece = newPiece;
            if (this.isLocking) {
                this.isLocking = false;
    ...
    clearLines() {
        ...
        if (linesCleared > 0) {
            ...
            (_d = (_c = this.callbacks).onLinesCleared) === null || _d === void 0 ? void 0 : _d.call(_c, linesCleared);
        }
    }
    render() {
        ...
        for (let y = 0; y < this.config.gridHeight; y++) {
            for (let x = 0; x < this.config.gridWidth; x++) {
                ...
                // Grid lines
                this.add.rectangle(cellX, cellY, this.config.cellSize, this.config.cellSize, parseInt(this.config.gridColor.replace('#', ''), 16)).setStrokeStyle(1, 0x666666);
                // Filled cells
                if (this.board[y][x] !== 0) {
                    this.add.rectangle(cellX, cellY, this.config.cellSize - 2, this.config.cellSize - 2, this.board[y][x]);
                }
            }
        }
        // Draw current piece
            ...
            for (let y = 0; y < this.currentPiece.shape.length; y++) {
                for (let x = 0; x < this.currentPiece.shape[y].length; x++) {
                    if (this.currentPiece.shape[y][x]) {
                        ...
                        this.add.rectangle(cellX, cellY, this.config.cellSize - 2, this.config.cellSize - 2, this.currentPiece.color);
                    }
                }
            }
        }
